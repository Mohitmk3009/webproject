<?php
	...
	// 	$row = array();
	// 	$query = "SELECT book_name, book_image FROM products ORDER BY book_name DESC";
	...
	function getBookByname($conn, $name){
		$query = "SELECT book_name, book_author, book_price FROM products WHERE book_name = '$name'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getOrderId($conn, $user_id){
		$query = "SELECT orderid FROM orders WHERE customerid = '$user_id'";
		$result = mysqli_query($conn, $query);
		if(!$result){
	...
	function getbookprice($name){
		$conn = db_connect();
		$query = "SELECT book_price FROM products WHERE book_name = '$name'";
		$result = mysqli_query($conn, $query);
		if(!$result){
	...
	function getAutName($conn, $autid){
		$query = "SELECT author_name FROM authors WHERE author_id = '$autid'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getCatName($conn, $catid){
		$query = "SELECT category_name FROM categories WHERE category_id = '$catid'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
